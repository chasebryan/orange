...
The hosted-control settings remain bound to the 2026-07-11 readback; the S2
...
Authoritative S2 `main` contains the Orange lexer/CLI foundation, normative
minimal Orange 2026 grammar, bounded parser, and pinned Rust toolchain
dependency. It has no type checker, semantic Core, proof checker, code
generator, cryptography package, distribution channel, or product release.
...
| Actions policy | Enabled at the 2026-07-11 readback; exactly six repository Action identities selected; broad GitHub-owned and verified-publisher allowances disabled; full-SHA pinning required; all external fork runs need approval | Source identities and mutable references are restricted. PR #6's required checks and exact S2 trusted-main push runs Required CI `29188111313`, Workflow Online Audit `29188111278`, External Links `29188111303` attempt 2, and Scorecard `29188111302` succeeded. This is an execution refresh, not a settings readback; scheduled trigger execution remains unproven. |
...
| CodeQL | Successful default-setup execution for Python, GitHub Actions, and Rust at exact S2 revision `52a3460853636f7cbaa27f3e27d86e032e3c82d4` | Run `29188111040` completed without analysis errors or warnings. Actions `1468459678`=`0/23`, Python `1468459893`=`0/50`, and Rust `1468460793`=`0/27`. Alerts #11-#17 are fixed rather than dismissed and no CodeQL alerts remain open; open alerts #4-#10 are Scorecard posture findings. No CodeQL threshold/ruleset or failing-alert merge-block proof exists, and the execution is not a fresh settings readback or all-code-scanning-clear claim. |
...
| OSPS-BR-01.01 | `Observed` with trigger-scope residual | EV-OPS-01 is merged and does not interpolate event metadata into shell commands; workflow expressions used for concurrency are not executed as shell source. PR #6 required checks and the named trusted-main push workflows succeeded at exact S2 revision `52a3460853636f7cbaa27f3e27d86e032e3c82d4`. | Scheduled and manual trigger execution remains unproven. Any future manual or event metadata reaching an interpreter must be allow-listed or safely passed through environment variables or arguments. |
...
| OSPS-DO-07.01 | `Partial` for pre-alpha compiler | On authoritative S2 `main`, `compiler/README.md` documents the dependency-free Rust lexer/parser/CLI workspace, and `make check` runs locked offline formatting, lint, documentation, unit, CLI, malformed, Unicode, line-ending, resource, and repeatability tests. No type checker, proof checker, code generator, or cryptography implementation exists. | Add clean-platform observations and update instructions with every merged capability; repository/compiler checks are not release assurance. |
...
| OSPS-QA-06.01 | `Observed` for repository-policy scope | EV-OPS-01/02 define merged repository policy tests before acceptance; S2 policy `0.2.1` has 88 passing Python tests, conformance validation succeeds, and Required CI `29188111313` passed at exact revision `52a3460853636f7cbaa27f3e27d86e032e3c82d4`. PR #6 final Required CI `29188056038` and Dependency Review `29188056060` also succeeded. Negative local mutations demonstrate fail-closed validator behavior. | Retain the required check and expand the suite with every product component. This is repository-policy evidence, not product implementation assurance. |
...
| OSPS-SA-03.01 | `Conditional` with early assessment | No release exists. EV-POL-04 covers repository controls and the merged pre-alpha lexer/parser/CLI boundary, not a full product or deployment assessment. | Repeat against executable code, dependencies, deployments, findings, and test results before each release-bearing capability. |
...
| OSPS-VM-05.03 | `Partial` | EV-OPS-02 supplies merged required PR dependency review plus Dependabot configuration, vulnerability/license checks, and green S2 trusted-main Scorecard run `29188111302`. | Dependabot operation and rejection of a known-vulnerable, malicious, or otherwise untrusted dependency remain unverified. Add explicit malicious-dependency policy and ecosystem coverage. Scorecard posture is not SAST or reproducible-build assurance. |
...
| OSPS-VM-06.02 | `Partial` | EV-GH-05 records successful CodeQL run `29188111040` at exact S2 revision `52a3460853636f7cbaa27f3e27d86e032e3c82d4` without analysis errors or warnings: Actions `1468459678`=`0/23`, Python `1468459893`=`0/50`, and Rust `1468460793`=`0/27`. Alerts #11-#17 are fixed at `2026-07-12T09:51:03Z`, not dismissed. | Evidence covers only those analyses, languages, rules, and revision. Define an authoritative threshold and exception/expiry policy, bind an appropriate CodeQL result to the ruleset, and demonstrate that a qualifying alert blocks merge. Fixed findings and zero results are not vulnerability-absence proof. Scorecard is posture data, not source-code weakness analysis or reproducible-build assurance. |
...
5. **Code-scanning enforcement gap:** CodeQL completed exact Actions, Python,
   and Rust analyses at S2 revision
   `52a3460853636f7cbaa27f3e27d86e032e3c82d4` without analysis errors or
   warnings and with zero results. Alerts #11-#17 are fixed rather than
   dismissed. No SAST remediation threshold, CodeQL-required ruleset context,
...
7. **Operational-evidence residual:** S2 is merged at exact revision
   `52a3460853636f7cbaa27f3e27d86e032e3c82d4`; trusted-main push runs succeeded
   for Required CI `29188111313`, Workflow Online Audit `29188111278`, External
   Links `29188111303` attempt 2, OpenSSF Scorecard `29188111302`, and CodeQL
   `29188111040`. PR #6's final Required CI `29188056038`, Dependency Review
   `29188056060`, and CodeQL `29188055399` also succeeded. External Links attempt
   1 encountered a transient `slsa.dev` connection failure. This refresh did not
   re-read settings.
