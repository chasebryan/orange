...
| EV-REP-01 | Public Git URL and Git history identify source changes and authors. `git ls-files` plus file-type inspection found no generated executable. The steward-designated working brand images and revision-bound semantic-prism diagram are the only tracked binary assets. Each has an exact path, role, provenance statement, and SHA-256 admission in repository policy; the local inventories are [`assets/brand/manifest.json`](../../assets/brand/manifest.json) and [`docs/images/README.md`](../images/README.md). |
...
| OSPS-QA-05.02 | `Observed` with limitation | EV-REP-01 identifies eleven non-executable visual assets: ten working brand images and one revision-bound architecture diagram. Required CI closes their paths and verifies exact SHA-256 admissions; the local records preserve roles and owner-supplied provenance. Two originals retain C2PA claims identifying OpenAI-generated media. | The C2PA signatures were not independently verified, binary review does not prove copyright or trademark rights, the D-017 working name lacks public-name clearance, and D-018 outbound terms remain open. Require the same closed admission for every future binary corpus or generated artifact. |
...
| OSPS-DO-07.01 | `Partial` for pre-alpha compiler | On authoritative S3a `main`, `compiler/README.md` documents the dependency-free Rust lexer/parser/semantic/Core/evaluator/CLI workspace, and `scripts/ci/check-repository` runs locked offline formatting, lint, documentation, debug and optimized unit/CLI, malformed, Unicode, line-ending, resource, and repeatability tests from fresh cache roots. No proof checker, canonical Core, code generator, or cryptography implementation exists. | Add clean-platform observations and update instructions with every merged capability; repository/compiler checks are not release assurance. |
