...
Solo/pre-alpha assessment amendment and hosted-execution refresh: 2026-07-12.
The hosted-control settings remain bound to the 2026-07-11 readback; the S1
execution evidence below is a separate run/result refresh, not a fresh settings
observation.
...
Authoritative S1 `main` contains an incomplete Orange lexer/CLI compiler
foundation and a pinned Rust toolchain dependency. It has no parser, checker,
code generator, cryptography package, distribution channel, or product release.
S2 contains a candidate bounded parser and policy expansion, but those receive
no merged operating-evidence credit until an exact merge and post-merge checks
exist. Conditional release rows receive no compliance credit: `not triggered`
...
A separate 2026-07-12 execution refresh is bound to exact S1 `main` revision
`469bdec6037f20c8d099d61a09a3d19a55c88231`. It records PR #5 and post-merge
...
| EV-GH-05 | GitHub code-scanning execution refresh: CodeQL run `29186965847` completed at exact S1 `main` revision `469bdec6037f20c8d099d61a09a3d19a55c88231` without analysis errors or warnings. GitHub Actions analysis `1468413067` reported `0/23`, Python analysis `1468413065` reported `0/50`, and Rust analysis `1468413062` reported `7/27`. The seven Rust results are open high `rust/path-injection` alerts #11-#17, all classified `test` in `compiler/crates/orangec/tests/cli.rs`. S2 contains candidate remediation, but no closure is claimed before an exact S2 merge, post-merge CodeQL execution, and alert readback. This refresh proves only the named execution, analyses, language rule counts, and revision; it is not a fresh settings readback, CodeQL threshold, merge block, independent analysis, or vulnerability-absence proof. Scorecard posture SARIF is not SAST or reproducible-build assurance. |
...
| EV-OPS-01 | Merged workflows in [`.github/workflows/`](../../.github/workflows/) use full action SHAs, top-level empty permissions, explicit job permissions, timeouts, concurrency, non-persistent checkout credentials, and no `pull_request_target`. Exact S1 `main` revision `469bdec6037f20c8d099d61a09a3d19a55c88231` produced successful push runs Required CI `29186961911`, Workflow Online Audit `29186961920`, External Links `29186961966`, OpenSSF Scorecard `29186961902`, and CodeQL `29186965847`. These are exact push/trusted-main results; scheduled triggers remain unproven, and they do not refresh settings. |
| EV-OPS-02 | Merged [Dependabot configuration](../../.github/dependabot.yml), [dependency-review policy](../../.github/dependency-review-config.yml), repository validator, schemas, and conformance fixtures are authoritative on S1 `main`. Solo-bootstrap policy `0.2.0` has 78 passing Python tests, and post-merge Required CI `29186961911` passed at the exact S1 revision. PR #5 final Required CI `29186933765` and Dependency Review `29186933780` succeeded before merge; the dependency result remains a required producer-bound PR check. Dependabot operation and rejection of a known-vulnerable, malicious, or otherwise untrusted dependency have not been demonstrated. Scorecard `29186961902` passed on S1 `main`, but it reports project posture rather than SAST or reproducible-build assurance; public Scorecard publication and OIDC remain disabled. S2 policy `0.2.1` and its 88-test suite are candidate evidence only until merge. |
...
| Actions policy | Enabled at the 2026-07-11 readback; exactly six repository Action identities selected; broad GitHub-owned and verified-publisher allowances disabled; full-SHA pinning required; all external fork runs need approval | Source identities and mutable references are restricted. PR #5's required checks and exact S1 trusted-main push runs Required CI `29186961911`, Workflow Online Audit `29186961920`, External Links `29186961966`, and Scorecard `29186961902` succeeded. This is an execution refresh, not a settings readback; scheduled trigger execution remains unproven. |
...
| CodeQL | Successful default-setup execution for Python, GitHub Actions, and Rust at exact S1 revision `469bdec6037f20c8d099d61a09a3d19a55c88231` | Run `29186965847` completed without analysis errors or warnings. Actions `1468413067`=`0/23`, Python `1468413065`=`0/50`, and Rust `1468413062`=`7/27`. Open high Rust alerts #11-#17 are all test-classified `rust/path-injection` results in the CLI integration tests. S2 remediation is candidate only. No CodeQL threshold/ruleset or failing-alert merge-block proof exists, and the execution is not a fresh settings readback. |
...
| OSPS-BR-01.01 | `Observed` with trigger-scope residual | EV-OPS-01 is merged and does not interpolate event metadata into shell commands; workflow expressions used for concurrency are not executed as shell source. PR #5 required checks and the named trusted-main push workflows succeeded at exact S1 revision `469bdec6037f20c8d099d61a09a3d19a55c88231`. | Scheduled and manual trigger execution remains unproven. Any future manual or event metadata reaching an interpreter must be allow-listed or safely passed through environment variables or arguments. |
...
| OSPS-DO-07.01 | `Partial` for pre-alpha compiler | On authoritative S1 `main`, `compiler/README.md` documents the dependency-free Rust lexer/CLI workspace, and `make check` runs locked offline formatting, lint, documentation, unit, and CLI tests. S2 adds candidate parser documentation and tests, but no parser evidence is merged yet. No checker, code generator, or cryptography implementation exists. | Add clean-platform observations and update instructions with every merged capability; repository/compiler checks are not release assurance. |
...
| OSPS-QA-06.01 | `Observed` for repository-policy scope | EV-OPS-01/02 define merged repository policy tests before acceptance; S1 policy `0.2.0` has 78 passing Python tests, conformance validation succeeds, and Required CI `29186961911` passed at exact revision `469bdec6037f20c8d099d61a09a3d19a55c88231`. PR #5 final Required CI `29186933765` and Dependency Review `29186933780` also succeeded. Negative local mutations demonstrate fail-closed validator behavior. S2 policy `0.2.1` and 88 tests remain candidate evidence. | Retain the required check and expand the suite with every product component. This is repository-policy evidence, not product implementation assurance. |
...
| OSPS-SA-03.01 | `Conditional` with early assessment | No release exists. EV-POL-04 covers repository controls and authoritative S1 lexer/CLI behavior; its S2 amendment covers the candidate bounded parser. Neither is a full product or deployment assessment. | Repeat against executable code, dependencies, deployments, findings, and test results before each release-bearing capability. |
...
| OSPS-QA-06.02 | `Partial` | EV-POL-04 documents PR, merge, nightly, weekly, and release-candidate test classes. EV-OPS-01/02 provide merged repository-check commands, 78 passing S1 policy tests, successful PR #5 checks, and exact green S1 trusted-main push results. The S1 compiler defines formatting, lint, documentation, unit, CLI, malformed-input, and repeatability tests. S2's parser cases and 88-test policy suite remain candidate evidence until merge. | Scheduled cadence is not established by repository configuration alone. Bind each accepted compiler change to its exact required-CI result and publish exact environments, expected outputs, retention, and failure evidence as each future test class becomes operating. |
...
| OSPS-VM-05.03 | `Partial` | EV-OPS-02 supplies merged required PR dependency review plus Dependabot configuration, vulnerability/license checks, and green S1 trusted-main Scorecard run `29186961902`. | Dependabot operation and rejection of a known-vulnerable, malicious, or otherwise untrusted dependency remain unverified. Add explicit malicious-dependency policy and ecosystem coverage. Scorecard posture is not SAST or reproducible-build assurance. |
...
| OSPS-VM-06.02 | `Partial` | EV-GH-05 records successful CodeQL run `29186965847` at exact S1 revision `469bdec6037f20c8d099d61a09a3d19a55c88231` without analysis errors or warnings: Actions `1468413067`=`0/23`, Python `1468413065`=`0/50`, and Rust `1468413062`=`7/27`. Rust alerts #11-#17 are open high `rust/path-injection`, all test classified. | Evidence covers only those analyses, languages, rules, and revision. S2 contains candidate remediation, but closure requires its exact merge plus post-merge CodeQL and alert readback. Define an authoritative threshold and exception/expiry policy, bind an appropriate CodeQL result to the ruleset, and demonstrate that a qualifying alert blocks merge. Scorecard is posture data, not source-code weakness analysis or reproducible-build assurance. |
...
5. **Code-scanning enforcement gap:** CodeQL completed exact Actions, Python,
   and Rust analyses at S1 revision
   `469bdec6037f20c8d099d61a09a3d19a55c88231` without analysis errors or
   warnings. The Rust analysis has seven open high test-classified
   `rust/path-injection` alerts #11-#17. S2 contains candidate remediation, not
   closure. No SAST remediation threshold, CodeQL-required ruleset context,
   exception/expiry policy, or failing-alert merge-block proof exists
   (OSPS-VM-06.01/02). The evidence does not extend beyond the exact languages,
   rules, and revision. Scorecard is project posture, not SAST or
...
7. **Operational-evidence residual:** S1 is merged at exact revision
   `469bdec6037f20c8d099d61a09a3d19a55c88231`; trusted-main push runs succeeded
   for Required CI `29186961911`, Workflow Online Audit `29186961920`, External
   Links `29186961966`, OpenSSF Scorecard `29186961902`, and CodeQL
   `29186965847`. PR #5's final Required CI `29186933765` and Dependency Review
   `29186933780` also succeeded. This refresh did not re-read settings.
