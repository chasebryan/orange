...
| Control | Status | Exact current evidence | Gap and next evidence required |
| --- | --- | --- | --- |
| OSPS-AC-04.02 | `Candidate` | EV-CAND-01 grants read-only contents to validation jobs; only trusted-event Scorecard has `id-token: write` and `security-events: write`. | Complete static/hosted validation, protect workflow changes, document why each write is necessary, and re-review every permission delta. |
