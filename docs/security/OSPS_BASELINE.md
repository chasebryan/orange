...
The hosted-control settings remain bound to the 2026-07-11 readback; the S2 and
S3a execution evidence below is a separate run/result refresh, not a fresh
...
| Actions policy | Enabled at the 2026-07-11 readback; exactly six repository Action identities selected; broad GitHub-owned and verified-publisher allowances disabled; full-SHA pinning required; all external fork runs need approval | Source identities and mutable references are restricted. EV-OPS-03 records green PR #9 required checks and exact S3a trusted-main push runs. This is an execution refresh, not a settings readback; scheduled trigger execution remains unproven. |
...
| CodeQL | Successful configured analyses for Python, GitHub Actions, and Rust; exact zero-result readback remains bound to S2 while S3a executions are separately recorded | EV-GH-05 records the exact S2 zero-result and alert-state readback. EV-GH-07 records successful PR #9 and post-merge S3a CodeQL executions without claiming their result counts or alert state. No CodeQL threshold/ruleset or failing-alert merge-block proof exists, and neither execution refresh is a fresh settings readback or vulnerability-absence claim. |
...
| OSPS-BR-01.01 | `Observed` with trigger-scope residual | EV-OPS-01 is merged and does not interpolate event metadata into shell commands; workflow expressions used for concurrency are not executed as shell source. EV-OPS-03 records successful PR #9 and trusted-main push executions at exact S3a revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`. | Scheduled and manual trigger execution remains unproven. Any future manual or event metadata reaching an interpreter must be allow-listed or safely passed through environment variables or arguments. |
...
| OSPS-QA-06.01 | `Observed` for repository-policy scope | EV-OPS-01/02 retain the historical policy evidence; EV-OPS-03 records policy `0.2.3`, 95 passing Python tests, zero foundation-validator findings, PR #9 Required CI `29215790064`, and post-merge Required CI `29215877872` at exact S3a revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`. Negative local mutations demonstrate fail-closed validator behavior. | Retain the required check and expand the suite with every product component. This is repository-policy evidence, not product implementation assurance. |
...
| OSPS-QA-06.02 | `Partial` | EV-POL-04 documents PR, merge, nightly, weekly, and release-candidate test classes. EV-OPS-03 records merged repository-check commands, 89 Rust tests including the documentation test, 95 policy tests, successful PR #9 checks, and exact green S3a trusted-main push results. The S3a compiler covers formatting, lint, documentation, unit, CLI, malformed-input, parser, semantic, Core, evaluator, Unicode, line-ending, resource, and repeatability behavior. | Scheduled cadence is not established by repository configuration alone. Bind each accepted compiler change to its exact required-CI result and publish exact environments, expected outputs, retention, and failure evidence as each future test class becomes operating. |
...
| OSPS-VM-05.03 | `Partial` | EV-OPS-02 supplies merged required PR dependency review plus Dependabot configuration and vulnerability/license checks. EV-OPS-03 records green PR #9 Dependency Review `29215790110` and trusted-main Scorecard `29215877875`. | Dependabot operation and rejection of a known-vulnerable, malicious, or otherwise untrusted dependency remain unverified. Add explicit malicious-dependency policy and ecosystem coverage. Scorecard posture is not SAST or reproducible-build assurance. |
...
| OSPS-VM-06.02 | `Partial` | EV-GH-05 records the exact S2 zero-result and alert-state readback. EV-GH-07 records successful PR #9 CodeQL `29215789258` at head `8c48a85997b756cf65d64110ebc869bb26e49079` and post-merge dynamic CodeQL `29215877437` at exact S3a revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`, without a new result-count or alert-state claim. | Evidence covers only the named analyses, languages, rules, revisions, and events. Define an authoritative threshold and exception/expiry policy, bind an appropriate CodeQL result to the ruleset, and demonstrate that a qualifying alert blocks merge. Fixed findings and successful runs are not vulnerability-absence proof. Scorecard is posture data, not source-code weakness analysis or reproducible-build assurance. |
...
5. **Code-scanning enforcement gap:** EV-GH-05 records exact zero-result Actions,
   Python, and Rust analyses at S2 and the fixed-not-dismissed status of alerts
   #11-#17. EV-GH-07 records successful PR and post-merge S3a CodeQL executions
   without a new result-count or alert-state readback. No SAST remediation
   ...
   not extend beyond the named languages, rules, revisions, and events.
...
7. **Operational-evidence residual:** S3a is merged at exact revision
   `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`. PR #9 head
   `8c48a85997b756cf65d64110ebc869bb26e49079` passed Required CI
   `29215790064`, Dependency Review `29215790110`, and CodeQL `29215789258`.
   Trusted-main push runs then succeeded for Required CI `29215877872`, Workflow
   Online Audit `29215877891`, External Links `29215877874`, OpenSSF Scorecard
   `29215877875`, and dynamic CodeQL `29215877437`. This refresh did not re-read
   settings or refresh CodeQL result counts and alert state.
