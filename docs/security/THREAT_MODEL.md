...
The D-004 pre-epoch laboratory byte-materializes exactly 70 candidate-neutral
...
| Git commits and repository files | Fork/branch/PR or privileged push | TB-001, TB-003 | Repository scripts, workflow definitions, and the 70 byte-materialized D-004 suite-only subjects are security-sensitive even before product code. Integrity parsing of those subjects is not candidate execution or semantic evidence. |
