...
| TB-013 | Local caller and filesystem to CMP-004 | Command-line arguments, file names, file bytes, standard input, output streams, and operating-system errors | The CLI uses no network or third-party crate, normalizes exposed I/O errors, escapes displayed path and control text, validates UTF-8, caps each source at 16 MiB, and returns stable success/compilation/usage status classes. Its portable metadata/open/descriptor-metadata sequence rejects observed non-regular files; Unix additionally rejects an opened descriptor whose device/inode differs from the pre-open target. This does not provide race-free path or symlink confinement: replacement with a special file may block `open`, the same inode may be mutated during a read, and non-Unix hosts do not receive the identity comparison. Platform behavior remains security-sensitive. | Current pre-alpha |
