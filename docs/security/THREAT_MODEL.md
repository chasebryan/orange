...
Hosted-execution refresh: 2026-07-12 through the PR #9 acceptance path and
post-merge push at exact S3a `main`; no repository-setting readback was
...
bound to their named S2 or S3a revision and event.
...
- The authoritative public repository is
  ...
  default branch.
- The S3a `main` tree consists of the pre-alpha Rust compiler workspace and its
...
| CMP-003 | Repository CI | Current on `main` with exact acceptance evidence | Repository-owned policy, dependency-review, link, workflow-metadata-audit, and Scorecard workflows are under [`.github/workflows/`](../../.github/workflows/). PR #9 passed Required CI `29215790064`, Dependency Review `29215790110`, and CodeQL `29215789258` before merge as exact S3a `main` revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`; Required CI covered policy `0.2.3`, 89 Rust tests including the documentation test, 95 Python tests, and a zero-finding foundation validation. At that exact merged revision, push Required CI `29215877872`, Workflow Online Audit `29215877891`, External Links `29215877874`, Scorecard `29215877875`, and dynamic CodeQL `29215877437` also succeeded. Ruleset `18810248` requires the exact Required CI and Dependency Review app-bound contexts. These executions do not demonstrate semantic soundness, vulnerability absence, independent review, scheduled or manual-dispatch event behavior, or refreshed hosted settings. |
...
| CTL-004 | Full-commit-SHA and selected-source requirements in repository settings and [`DEPENDENCY_POLICY.md`](../../DEPENDENCY_POLICY.md); current workflows pin repository Actions to 40-character SHAs and execute Scorecard directly at `sha256:2dd6a6d60100f78ef24e14a47941d0087a524b4d3642041558239b1c6097c941` | 2026-07-11 platform settings plus current policy and S3a `main` source; Required CI `29215877872`, Workflow Online Audit `29215877891`, External Links `29215877874`, and Scorecard `29215877875` succeeded at exact revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5` | The six Action repositories and separately admitted Scorecard image remain upstream trust. Content addressing does not make their code trustworthy, mirror it, or eliminate publisher, registry, provenance, runner, and host compromise. The execution refresh did not re-read settings. |
...
| CTL-006 | PR workflows use `pull_request`, no `pull_request_target`, no configured repository or environment secrets, only a job-scoped `GITHUB_TOKEN` limited to `contents: read`, bounded timeouts, concurrency, and checkout with `persist-credentials: false` | Current `main` source; exact Required CI and Dependency Review contexts are enforced by ruleset `18810248`; PR #9 runs `29215790064` and `29215790110` succeeded before S3a merged as `6c0bd3021cf2df603e08808e4660724ca1e2b2a5` | PR content includes executable repository scripts; runner and Action compromise remain external assumptions, and sole-owner merge is not independent review. |
...
| CTL-019 | GitHub CodeQL default setup analyzes the current Actions, Python, and Rust surfaces and preserves alert lifecycle state | PR #9 CodeQL `29215789258` succeeded before S3a merge, and dynamic CodeQL `29215877437` succeeded on the exact merged revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`. The earlier exact S2 run `29188111040` completed without analysis errors or warnings; Actions `1468459678`=`0/23`, Python `1468459893`=`0/50`, and Rust `1468460793`=`0/27`, and alerts #11-#17 were fixed without dismissal. | Coverage is limited to the named revisions, languages, configured rules, queries, and platform executions. Successful CodeQL runs and earlier zero results do not mean all code-scanning alerts are closed or prove vulnerability absence. No CodeQL threshold or independent analysis exists. |
| CTL-020 | The dependency-free Rust compiler frontend forbids unsafe code; caps source bytes, tokens, syntax nodes, parser events, emitted diagnostics, and recovery depth; sanitizes diagnostic control text; and exercises source, span, lexer, parser, diagnostic, and CLI behavior with malformed-input, Unicode, line-ending, resource, and repeatability tests. | Policy `0.2.3`, 89 Rust tests including the documentation test, parser source, and exact budget bindings are merged at revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`; PR Required CI `29215790064` and post-merge Required CI `29215877872` succeeded | Tests do not establish parser correctness, semantic soundness, proof, code-generation, cryptographic, leakage, or production correctness; the Rust toolchain, host, algorithmic behavior inside the budgets, and sole-owner review remain trusted or residual risks. |
| CTL-021 | D-026, Accepted OEP-0003, and [`SEMANTICS_2026.md`](../SEMANTICS_2026.md) constrain S3a to same-kind namespace uniqueness, exact contextual `Int`/`Word[8]` typed literals, fail-closed semantic/Core/integer/evaluation budgets, a noncanonical source-ordered Typed Reference Core, and deterministic all-or-nothing evaluation output. | Accepted implementation evidence at exact merged revision `6c0bd3021cf2df603e08808e4660724ca1e2b2a5`: policy `0.2.3`, 89 Rust tests including the documentation test, 95 Python tests, and zero foundation-validator findings passed PR Required CI `29215790064`; Dependency Review `29215790110` and CodeQL `29215789258` also passed. Post-merge Required CI `29215877872` and dynamic CodeQL `29215877437` succeeded at exact `main`. | Shared specification/implementation mistakes, integer decoding and formatting defects, allocation or host failure within budgets, and sole-owner review remain. Acceptance records merged, tested implementation evidence only; they do not establish semantic proof, independent review, refinement, code generation, leakage, ABI, cryptographic correctness, external certification, or product-release readiness. |
...
| TM-002 | ADV-001/002; TB-001/004; AS-003/004 | Attacker-controlled workflow, script, or metadata reaches a privileged runner and steals credentials or changes state. | CTL-005/006/019; current PR jobs declare read-only source access, no configured repository or environment secrets, only a job-scoped read-only `GITHUB_TOKEN`, timeouts, and no `pull_request_target`. Ruleset `18810248` requires the exact app-bound Required CI and Dependency Review contexts. PR #9 Required CI `29215790064`, Dependency Review `29215790110`, and CodeQL `29215789258` succeeded; post-merge Required CI `29215877872`, Workflow Online Audit `29215877891`, External Links `29215877874`, Scorecard `29215877875`, and dynamic CodeQL `29215877437` succeeded at exact S3a `main`. | Policy-check every workflow diff; keep privileged jobs off PR events; validate metadata before shell use; require CI and security review on workflow paths. | Low now: demonstrated PR jobs have read-only permissions; reassess every permission or event change | High | Medium | Actions/runner isolation, sole-owner review, query coverage, open Scorecard posture alerts, and future permission edits remain trusted or residual risks. Green runs do not prove vulnerability absence. | CI/Release authority, currently Bootstrap Steward; every workflow change; `open-current` |
