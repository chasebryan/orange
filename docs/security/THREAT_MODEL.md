...
unreviewed changes to an unprotected default branch, unsafe CI evolution,
credential disclosure, and planning text that overstates controls which do not
yet exist. The intended product will add much higher-consequence proof,
...
- The current foundation change contains candidate GitHub Actions workflows for
  repository policy, dependency review, link checking, and OpenSSF Scorecard.
  They are not authoritative or operating evidence until merged, executed
  successfully at the reviewed revision, and protected where required.
...
| CMP-003 | Repository CI | Branch candidate; not yet authoritative | Proposes policy, dependency-review, link, and Scorecard workflows under GitHub Actions. Workflow source is under [`.github/workflows/`](../../.github/workflows/); hosted results and required-check enforcement are pending. |
...
| --- | --- | --- | --- |
| CTL-001 | Public Git history, structured issue forms, pull-request template, and CODEOWNERS in [`.github/`](../../.github/) | Current, partly platform-enforced | CODEOWNERS has only the author/owner and cannot provide independent approval. `main` is not protected. |
...
| CTL-004 | Full-commit-SHA and selected-source requirements in repository settings and [`DEPENDENCY_POLICY.md`](../../DEPENDENCY_POLICY.md); candidate workflows pin repository Actions to 40-character SHAs and execute Scorecard directly at `sha256:2dd6a6d60100f78ef24e14a47941d0087a524b4d3642041558239b1c6097c941` | Current platform settings and policy; branch-candidate workflow source | The six Action repositories and separately admitted Scorecard image remain upstream trust. Content addressing does not make their code trustworthy, mirror it, or eliminate publisher, registry, provenance, runner, and host compromise. |
| CTL-005 | Repository workflow token default is read-only; each candidate workflow begins with `permissions: {}` and grants per-job minimums | Current platform default; branch-candidate source | Scorecard needs `id-token: write` and `security-events: write`; that privileged job must remain off untrusted events. |
| CTL-006 | Candidate PR workflows use `pull_request`, no `pull_request_target`, no repository secrets, bounded timeouts, concurrency, and checkout with `persist-credentials: false` | Branch candidate; no hosted execution or enforcement yet | PR content includes executable repository scripts; runner and action compromise remain external assumptions. |
...
| CTL-008 | [`CONTRIBUTING.md`](../../CONTRIBUTING.md) blocks third-party merge until licensing terms close and requires branch, PR, evidence, and impact review | Current documented constraint | Repository settings do not yet enforce the full workflow; legal decision D-018 remains blocked. |
...
| CTL-010 | [`DEPENDENCY_POLICY.md`](../../DEPENDENCY_POLICY.md), candidate Dependabot/dependency-review configuration, and candidate Scorecard workflow define admission and surveillance | Current policy; branch-candidate automation with no hosted results | No product dependency graph exists; future manifests, SCA exceptions, VEX, and admission records remain to be implemented. |
...
| Trusted Actions runs | Push, schedule, or manual dispatch | TB-005 | Scorecard can mint OIDC and upload SARIF; event restrictions and minimum permissions are critical. |
...
1. **TM-001 — corrupt the authoritative plan:** ADV-002 compromises the sole
   owner, crosses TB-002, directly updates unprotected `main` across TB-003, and
   changes a decision or security rule without independent review. Consumers
   mistake the altered repository for authorized project direction.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TM-001 | ADV-002/004/008; TB-002/003; AS-001/002/003/011 | Compromise or unilateral authority changes or erases authoritative decisions, settings, or history. | CTL-001, CTL-007, CTL-008; public history aids detection but not prevention. | Protect `main`; require PRs, passing checks, conversation resolution, no force push/deletion, and no bypass. Add a non-author maintainer before claiming independent review; establish recovery and access review. | High: one owner and no branch rule | High | High | Owner or platform compromise can still abuse admin recovery; social review remains necessary. | Bootstrap Steward; each rules/access change and quarterly; `open-current` |
| TM-002 | ADV-001/002; TB-001/004; AS-003/004 | Attacker-controlled workflow, script, or metadata reaches a privileged runner and steals credentials or changes state. | CTL-005/006; candidate PR jobs have read-only source access, no secrets, timeouts, and no `pull_request_target`. | Policy-check every workflow diff; keep privileged jobs off PR events; validate metadata before shell use; require CI and security review on workflow paths. | Low now: no workflow is authoritative on `main`; reassess after merge | High | Medium | Actions/runner isolation and future permission edits remain trusted. | CI/Release authority, currently Bootstrap Steward; every workflow change; `candidate-control` |
| TM-003 | ADV-003; TB-005/012; AS-004/009 | Compromised action, tool, image, registry, archive, or dependency falsifies results or executes malicious code. | CTL-004/010; current selected-action/full-SHA settings plus candidate full SHAs, the content-addressed Scorecard runtime, download digests, and dependency review. | Preserve the minimum allowlist and exact container admission; retain provenance/admission records; archive dependencies; verify signatures where available; use hermetic release inputs and independent rebuilds. | Low now; Medium after candidate third-party CI lands | High | High | A selected full SHA or digest identifies malicious bytes as faithfully as good bytes; admission, publisher provenance, registry availability, runner integrity, and monitoring remain human or external judgments. | Dependency and CI authority, currently Bootstrap Steward; each admission/update and weekly surveillance; `candidate-control` |
