...
Compiler-lineage snapshot: S1 merged as exact revision
`469bdec6037f20c8d099d61a09a3d19a55c88231`; S2 parser evidence remains bound
to its pending merge revision

Hosted-execution refresh: 2026-07-12 at exact S1 `main`; no repository-setting
...
Authoritative S1 `main` includes a local pre-alpha Rust compiler foundation with
source-file, lexer, diagnostic, and CLI input surfaces. This S2 change adds a
candidate bounded parser and normative minimal grammar; it is not merged
operating evidence yet. Orange has no type or name analysis, semantic Core,
...
disclosure, and compiler-toolchain compromise. The S2 candidate also exposes
parser recovery, tree, and parser-budget risks. Repository and governance risks
...
- The authoritative public repository is
  ...
  default branch.
- The S2 candidate tree consists of the pre-alpha Rust compiler workspace and
  its normative Orange 2026 lexical/grammar specification, planning,
  governance, security policy, issue and pull-request templates, historical
  Gate 0 schemas, conformance material, and repository automation. S1 `main`
  does not yet contain the parser or normative grammar. The schemas remain
...
- PR #5's final Required CI `29186933765` and Dependency Review `29186933780`
  succeeded before squash merge as exact S1 `main` revision
  `469bdec6037f20c8d099d61a09a3d19a55c88231`. Its post-merge push runs also
  succeeded: Required CI `29186961911`, Workflow Online Audit `29186961920`,
  External Links `29186961966`, OpenSSF Scorecard `29186961902`, and CodeQL
  `29186965847`. Required CI exercised S1 policy `0.2.0` and all 78 Python
  tests. S2 policy `0.2.1` and 88 tests remain candidate evidence until merge.
- CodeQL run `29186965847` completed without analysis errors or warnings.
  Actions analysis `1468413067` reported `0/23`, Python analysis `1468413065`
  reported `0/50`, and Rust analysis `1468413062` reported `7/27`. The seven
  Rust results are open high `rust/path-injection` alerts #11-#17, all
  classified `test` in `compiler/crates/orangec/tests/cli.rs`. S2 contains
  candidate remediation that moves affected tests off environment-derived
  temporary paths; closure requires an exact merge, post-merge CodeQL result,
  and alert readback.
...
- elaborator, semantic cores, proof search, canonical Proof IR, and the
  authoritative offline checker beyond the current lexical and candidate syntax
  boundaries;
...
| CMP-003 | Repository CI | Current on `main` with exact-revision hosted evidence | Repository-owned policy, dependency-review, link, workflow-metadata-audit, and Scorecard workflows are under [`.github/workflows/`](../../.github/workflows/). At exact S1 `main` revision `469bdec6037f20c8d099d61a09a3d19a55c88231`, Required CI `29186961911`, Workflow Online Audit `29186961920`, External Links `29186961966`, Scorecard `29186961902`, and CodeQL `29186965847` succeeded. Ruleset `18810248` requires the exact Required CI and Dependency Review app-bound contexts. These push executions do not separately demonstrate scheduled or manual-dispatch event behavior and do not refresh hosted settings. |
| CMP-004 | Orange driver and language services | Current S1 lexical/CLI slice; candidate S2 syntax slice | S1 `main` has the Rust source map, byte spans, lexer, diagnostics, and `orangec check`/`lex`. S2 adds the bounded parser, syntax tree, and grammar as candidate evidence. No type or name analysis, elaborator, semantic validation, LSP, or code generation exists. |
...
| TB-008 | Surface source through CMP-004 to future CMP-005 Core and claims | File or standard-input UTF-8 bytes, paths, tokens, candidate syntax trees, diagnostics, later types, assumptions, and canonical serialization | S1 validates UTF-8, assigns checked byte spans, lexes, and emits structured diagnostics. S2's candidate parser recognizes the normative minimal Orange 2026 grammar and adds positive, malformed, Unicode, line-ending, resource, and repeatability tests. Name/type analysis, normative semantics, checked formats, and Core do not exist. Owner review is not independent. | Current through lexical boundary; candidate through syntax; future-blocking beyond it |
...
| ADV-005 | A malicious source, proof, certificate, package, object, or evidence author targets lexers, parsers, semantics, resource limits, claim binding, and compiler transitions. | Can reach the current S1 lexer/CLI locally and the S2 candidate parser when that branch is built; no merged Orange parser, checker, package client, or code generator exists yet. |
...
| CTL-004 | Full-commit-SHA and selected-source requirements in repository settings and [`DEPENDENCY_POLICY.md`](../../DEPENDENCY_POLICY.md); current workflows pin repository Actions to 40-character SHAs and execute Scorecard directly at `sha256:2dd6a6d60100f78ef24e14a47941d0087a524b4d3642041558239b1c6097c941` | 2026-07-11 platform settings plus current S1 policy and `main` source; Required CI `29186961911`, Workflow Online Audit `29186961920`, External Links `29186961966`, and Scorecard `29186961902` succeeded at exact S1 | The six Action repositories and separately admitted Scorecard image remain upstream trust. Content addressing does not make their code trustworthy, mirror it, or eliminate publisher, registry, provenance, runner, and host compromise. The execution refresh did not re-read settings. |
...
| CTL-006 | PR workflows use `pull_request`, no `pull_request_target`, no configured repository or environment secrets, only a job-scoped `GITHUB_TOKEN` limited to `contents: read`, bounded timeouts, concurrency, and checkout with `persist-credentials: false` | Current `main` source; exact Required CI and Dependency Review contexts are enforced by ruleset `18810248`; PR #5 final runs `29186933765` and `29186933780` succeeded before S1 merged | PR content includes executable repository scripts; runner and Action compromise remain external assumptions, and sole-owner merge is not independent review. |
...
| CTL-018 | The exact [`assets/brand/`](../../assets/brand/) inventory, owner-specific CODEOWNERS route, byte-level manifest, binary Git attributes, and repository-policy SHA-256 admissions protect the steward-designated working identity assets | Current S1 `main`; policy `0.2.0` and all 78 Python tests passed in Required CI `29186961911` at exact revision `469bdec6037f20c8d099d61a09a3d19a55c88231` | D-017 authorizes the working identity but does not provide public-name clearance, and D-018 outbound terms remain open; C2PA claims are preserved but not independently verified, content addressing does not prove rights or safe decoder behavior, and sole stewardship supplies no independent visual or rights review. |
| CTL-019 | GitHub CodeQL default setup analyzes the current Actions, Python, and Rust surfaces and preserves alert lifecycle state | At exact S1 `main`, run `29186965847` completed without analysis errors or warnings; Actions `1468413067`=`0/23`, Python `1468413065`=`0/50`, and Rust `1468413062`=`7/27`. Rust alerts #11-#17 are open high `rust/path-injection`, all classified `test` in the CLI integration tests. | Coverage is limited to the named revision, languages, configured rules, queries, and platform execution. S2 remediation is candidate only; fixed/closed status requires post-merge CodeQL and alert readback. No CodeQL threshold or independent analysis exists. |
| CTL-020 | S1's dependency-free Rust compiler foundation forbids unsafe code, caps source bytes, tokens, and emitted diagnostics, sanitizes diagnostic control text, and exercises source, span, lexer, diagnostic, and CLI behavior. S2 additionally caps syntax nodes, parser events, parser diagnostics, and recovery depth and adds parser/Unicode/line-ending/resource/repeatability cases. | S1 policy `0.2.0`, 78 tests, and compiler source are merged at exact revision `469bdec6037f20c8d099d61a09a3d19a55c88231`; S2 policy `0.2.1`, 88 tests, and parser evidence remain candidate until exact merge and hosted checks | Tests do not establish parser correctness, name/type or semantic correctness, proof, code-generation, cryptographic, leakage, or production correctness; the Rust toolchain, host, algorithmic behavior inside the budgets, and sole-owner review remain trusted or residual risks. |
...
| Current S1 lexer/CLI and candidate S2 parser | File paths, file bytes, standard input, command arguments, tokens, candidate trees, and output streams | TB-008, TB-013 | S1 must reject invalid UTF-8, malformed tokens, oversized input, ambiguous options, and resource exhaustion. S2 additionally must reject Unicode-confusable syntax, malformed grammar, recovery stalls, and parser resource exhaustion with bounded stable diagnostics. |
...
| TM-002 | ADV-001/002; TB-001/004; AS-003/004 | Attacker-controlled workflow, script, or metadata reaches a privileged runner and steals credentials or changes state. | CTL-005/006/019; current PR jobs declare read-only source access, no configured repository or environment secrets, only a job-scoped read-only `GITHUB_TOKEN`, timeouts, and no `pull_request_target`. Ruleset `18810248` requires the exact app-bound Required CI and Dependency Review contexts. PR #5 runs `29186933765` and `29186933780` succeeded; post-merge Required CI `29186961911` passed S1 policy `0.2.0` and all 78 tests. CodeQL `29186965847` completed without analysis errors/warnings but reported seven open high test-classified Rust path-injection results. | Policy-check every workflow diff; keep privileged jobs off PR events; validate metadata before shell use; require CI and security review on workflow paths. | Low now: demonstrated PR jobs have read-only permissions; reassess every permission or event change | High | Medium | Actions/runner isolation, sole-owner review, open CodeQL findings, query coverage, and future permission edits remain trusted or residual risks. | CI/Release authority, currently Bootstrap Steward; every workflow change; `open-current` |
...
| TM-014 | ADV-005/007/008; TB-008/013; AS-003/013 | Hostile source or path input exhausts resources, stalls candidate parser recovery, panics, produces incorrect tokens/spans/trees, leaks host details, or is accepted with hidden lexical or syntax errors. | CTL-020; S1 has UTF-8 validation, checked source IDs/spans, a 16 MiB source cap, token/diagnostic caps, sanitized output, normalized I/O errors, no unsafe code, no third-party crates, and positive/malformed/resource/repeatability tests. S2 candidate adds exact ASCII rules, node/event/parser-diagnostic/recovery caps, trees, and parser/Unicode/line-ending tests. | Retain every frontend budget; close or evidence-dispose CodeQL alerts #11-#17; extend mutation, fuzz/property, broken-output, and cross-implementation cases as the grammar grows; bind any semantic consumer to the exact accepted tree rather than reparsing. | Medium: the local CLI intentionally accepts attacker-controlled files; the parser is candidate until merge | Medium | Medium | Rust allocation failure within a cap, algorithmic complexity, shared specification/implementation mistakes, toolchain defects, host filesystem behavior, and untested platform differences remain. | Project owner; every frontend/input-boundary change; `open-current` |
...
| `docs/ARCHITECTURE.md` | Defines the current/candidate parser and future checker, compiler, ABI, package, registry, and evidence boundaries. | TM-005 through TM-012, TM-014 |
