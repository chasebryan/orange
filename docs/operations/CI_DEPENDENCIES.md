...
| Git | Checkout implementation and validator repository inventory | The first-party validator selects `/usr/bin/git`, fixes its child-command lookup to `/usr/bin:/bin`, requires a literal local `.git` directory with regular local config/index files and without common-directory or object alternates, and clears other Git configuration and repository-discovery variables; the runner-provided executable's exact version, package provenance, license, and binary digest are not captured |
