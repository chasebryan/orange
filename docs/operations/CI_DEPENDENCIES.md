...
| `cmp`, `curl`, `sha256sum`, `tar`, `stat`, `install`, `mktemp`, `rm`, `uname`, and `unshare` | Compare captured sources; download, verify, extract, inspect, install, clean temporary tools, and isolate copied commands in private user, mount, PID, `/proc`, and network namespaces | Runner-provided system tools; exact versions, provenance, package licenses, and binary digests are not captured. Namespace availability and enforcement depend on the mutable runner kernel and policy |
...
| --- | --- | --- | --- |
| `scripts/ci/check-repository` | Repository bytes, Cargo lock, selected Rust version, allowlisted Python and Cargo environments, fresh bytecode/build cache roots, and private user, mount, PID, `/proc`, and network namespaces for copied commands; Cargo also runs locked/offline after the toolchain exists | Rust formatting, lint, docs, debug and optimized unit/CLI tests, repository invariants, and synthetic/adversarial fixture expectations pass under the executing toolchain and OS without copied-code visibility of the trusted parent process or a new external network path | Rust archives, Python, shell, Make, kernel, namespace implementation, filesystem, and host tools are not content-fixed; no signed execution manifest or independently replayable environment is emitted |
