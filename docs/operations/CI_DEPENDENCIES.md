...
| --- | --- | --- | --- |
| `scripts/ci/check-repository` | Repository bytes, Cargo lock, selected Rust version, allowlisted Python and Cargo environments, fresh bytecode/build cache roots, and private mount, PID, `/proc`, and network namespaces for copied commands; the gate uses an unprivileged user namespace where available or a privilege-dropping namespace supervisor otherwise, and Cargo also runs locked/offline after the toolchain exists | Rust formatting, lint, docs, debug and optimized unit/CLI tests, repository invariants, and synthetic/adversarial fixture expectations pass under the executing toolchain and OS without copied-code visibility of the trusted parent process, effective capabilities, privilege-reacquisition permission, or a new external network path | Rust archives, Python, shell, Make, kernel, namespace and privilege-drop implementations, filesystem, and host tools are not content-fixed; no signed execution manifest or independently replayable environment is emitted |
