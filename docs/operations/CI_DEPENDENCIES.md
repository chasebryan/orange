...
| Git | Checkout implementation and validator repository inventory | The first-party validator selects `/usr/bin/git` and clears Git configuration and repository-discovery variables; the runner-provided executable's exact version, package provenance, license, and binary digest are not captured |
