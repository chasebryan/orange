...
use super::domain::{
    ...
    HARD_GATE_COUNT, HARD_GATE_STATES, INPUT_BINDINGS, InputBinding, InputBindingId, METRICS,
    NONCLAIMS, OWNER_SCOPES, PROTOCOL_COUNTS, PROTOCOL_GAPS, REQUIRED_CANDIDATE_CASES, TOOL_STATES,
};
use super::sha256;
use super::strict_json::{self, JsonErrorKind, JsonValue};

const PACKET_ROOT_FIELDS: [&str; 28] = [
    "schema_version",
    "suite_version",
    "status",
    "epoch",
...
fn validate_packet_shape(value: &JsonValue) -> Result<(), PacketError> {
    ...
    validate_object_array(root, "tool_states", &TOOL_STATE_FIELDS, TOOL_STATES.len())?;
...
fn draft_packet_value() -> JsonValue {
    strict_json::object([
        ...
        string_entry("schema_version", "d006-pre-epoch-packet-v0.1"),
        ("selection".to_owned(), JsonValue::Null),
        string_entry("status", "draft_unfrozen"),
        string_entry("suite_version", "d006-v0.2-draft"),
        ("tool_states".to_owned(), tool_states_value()),
    ])
}
...
fn case_input_index_value() -> JsonValue {
    strict_json::object([
        ...
        string_entry("owner_protocol_review", "none"),
        string_entry("schema_version", "d006-pre-epoch-case-input-index-v0.1"),
...
fn tool_states_value() -> JsonValue {
    JsonValue::Array(
        TOOL_STATES
            .into_iter()
            .map(|tool| {
                strict_json::object([
                    string_entry("acquisition_status", "not_performed"),
                    string_entry("admission_status", "absent"),
                    string_entry("candidate", tool.candidate.as_str()),
                    ("content_sha256".to_owned(), JsonValue::Null),
                    ("dependency_graph_sha256".to_owned(), JsonValue::Null),
                    string_entry("execution_status", "not_performed"),
                    string_entry("installation_status", "not_performed"),
                    string_entry("name", tool.name),
                    ("version".to_owned(), JsonValue::Null),
                ])
            })
            .collect(),
    )
