...
use super::candidate_mappings::CANDIDATE_MAPPING_CATALOG_CANONICAL_SHA256;
...
const ROOT_FIELDS: [&str; 29] = [
    ...
    "candidate_mapping_catalog_sha256",
...
pub(crate) const CANDIDATE_MAPPING_CATALOG_SHA256: &str =
...
fn validate_packet(value: &JsonValue) -> Result<(), PacketError> {
    ...
    require_exact_string(root, "schema_version", "d004-pre-epoch-packet-v0.5", "$")?;
...
fn draft_packet_value() -> JsonValue {
    ...
    let mutation_ids = MUTATIONS.map(|mutation| mutation.id);
    strict_json::object([
        string_entry("schema_version", "d004-pre-epoch-packet-v0.5"),
